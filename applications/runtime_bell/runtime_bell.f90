...
! Build a Bell circuit, submit it to IBM Quantum, read the samples back.
! Needs IBM Quantum credentials in the environment. The circuit is submitted as
! built (no transpilation bound yet), so a backend may reject it.
...
program runtime_bell
  ...
  ! 4. Submit a Sampler job.
  print '(/, a, i0, a)', "Submitting Sampler job (", shots, " shots)..."
  call service%run_sampler(job, backend, qc, shots=shots)
  ...
  ! All handles (service, backends, job, res, qc) free themselves on scope exit.
